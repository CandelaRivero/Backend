...
import prods from '../daos/ProductosDao.js'
import session from 'express-session';
import passport from 'passport';


const { options } = require("./options/mysql.js.js.js.js")
...
async function CRUD() {
  ...
  const query = db.collections('usuarios')


  try {
    let id = 1
    let doc = query.doc(`${id}`)
    await doc.create( { nombre: 'Jose', dni: 123455566})
    id++
     doc = query.doc(`${id}`)
    await doc.create( { nombre: 'Marta', dni: 1222330099})
